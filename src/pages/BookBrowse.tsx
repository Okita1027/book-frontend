...
const BookBrowse: React.FC = () => {
  ...
  const handleSearch = () => {
    ...
    if (searchForm.authorName?.trim()) params.authorName = searchForm.authorName.trim();
    ...
    if (searchForm.publishedDateBegin?.trim()) params.publishedDateBegin = searchForm.publishedDateBegin.trim();
    if (searchForm.publishedDateEnd?.trim()) params.publishedDateEnd = searchForm.publishedDateEnd.trim();
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={8} lg={6}>
            ...
            <Select
              ...
              onChange={(value) => setSearchForm(prev => ({ ...prev, categoryId: value }))}
          ...
          <Col xs={24} sm={12} md={8} lg={6}>
            ...
            <Select
              ...
              onChange={(value) => setSearchForm(prev => ({ ...prev, publisherId: value }))}
              style={{ borderRadius: 8, width: '100%' }}
          ...
          <Col xs={24} sm={12} md={8} lg={6}>
            ...
            <DatePicker
              ...
              allowClear
              size="large"
              value={searchForm.publishedDateBegin ? dayjs(searchForm.publishedDateBegin) : null}
              onChange={(date) => setSearchForm(prev => ({ ...prev, publishedDateBegin: date ? date.format('YYYY-MM-DD') : '' }))}
              style={{ borderRadius: 8, width: '100%' }}
            />
          </Col>
          <Col xs={24} sm={12} md={8} lg={6}>
            ...
            <DatePicker
              ...
              allowClear
              size="large"
              value={searchForm.publishedDateEnd ? dayjs(searchForm.publishedDateEnd) : null}
              onChange={(date) => setSearchForm(prev => ({ ...prev, publishedDateEnd: date ? date.format('YYYY-MM-DD') : '' }))}
              style={{ borderRadius: 8, width: '100%' }}
            />
          </Col>
          <Col xs={24} sm={12} md={8} lg={6}>
                    ...
                    <div>
                      ...
                      <div style={{ marginBottom: 8 }}>
                        {book.categoryNames?.map((categoryName, index) => (
                          <Tag key={index} color="blue">{categoryName}</Tag>
